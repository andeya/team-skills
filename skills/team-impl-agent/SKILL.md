...
1. 读取 `docs/tasks/{slug}/` 下的 01-05 文件 + prompt-template.md（specAgent 产出）
2. 读取项目规范：`CLAUDE.md`、`AGENTS.md`、`CONTRIBUTING.md`
3. 读取 Constitutional Rules：从 orchestrator 继承的不可覆盖硬约束（人类介入、有向图回退、产出必须验证、Kill Switch、分期交付、自我约束预算、回退次数上限、验证先行原则）
...
3. **运行 CI 全量**：项目 CI 检查命令（参考 CLAUDE.md 或 05-risk.md §一验证计划中的具体命令）

> **验证协议**（步骤 1-3 每次声明"通过"前必须执行）：
>
