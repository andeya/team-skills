...
```
Step 1: 7 项硬门槛是否全部通过？（逐条检查，任一不通过则整体不通过）
Step 2: 5 个维度的证据收集全了吗？（Agent 1-5 的扫描结果）
Step 3: 每个二级验收项的证据是什么？（文件路径 + 关键内容摘要）
Step 4: 根据证据给出得分（0 / 40% / 60% / 80% / 满分）
Step 5: 总分多少？等级是什么？
Step 6: 改进建议按什么优先级排列？（硬门槛 > 0 分项 > 低于 60% > 低于 80%）
```
