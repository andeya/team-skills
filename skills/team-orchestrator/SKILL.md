...
5. 遵守 Constitutional Rules（`_team-rules/constitutional-rules.md`），不可跳过任何规则
6. 如果用户指定 --compact 精简模式，简化 H1 为单句确认、简化 H2 为单句确认、跳过 Step 6，保留 H4 验收不可省略

关键区别：你不是线性流水线。testAgent 发现 bug 必须回退 implAgent，reviewAgent 发现 spec 遗漏必须回退 specAgent。同一阶段回退不超过 2 次。H1 和 H4 在任何模式下均不可省略。精简模式下 H1 和 H2 可简化为单句确认，但不可跳过。
```
...
回退修复要求：{如果是回退修复，要求修复过程遵循 TDD 循环，修复后更新 06-tdd-log.md 和 08-ai-decisions.md，并重新运行全量测试}
...
**测试/CI 门禁**：运行项目测试和 CI 检查命令（或验证 implAgent 提供的新鲜执行证据），确认退出码 = 0 且失败数 = 0。如果测试或 CI 未通过 → 回退 implAgent，附上失败输出和具体失败项，要求修复后更新 06-tdd-log.md 并重新提交。不可跳过失败进入 Step 4。
