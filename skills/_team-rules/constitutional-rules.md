...
2. **有向图回退** — 发现问题必须回退，禁止"先记着后面修"
   - **为什么（FP-4）**：声明不等于事实。"后面修"是一种声明——承诺未来会修复，但没有任何证据保证。问题在发现时最容易定位，延迟修复使上下文流失。测试失败是事实——忽略它继续后续流程不会让它消失，只会让它在更远的地方以更高的代价爆发
...
| "这个失败跟我的改动无关" | 验证无关性（git stash → 运行 → 仍失败 = 确认无关，记录到文档）；假设无关但不验证 = 掩盖问题 |
| "用户没要求写文档" | 文档是流程一部分 |
