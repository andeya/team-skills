...
- 只审查代码不看 Constitutional 合规
- 发现 P0/P1 问题不路由，自己修
- 资产更新不满足消费方契约（缺触发条件/可执行指令/示例）
- 复盘写泛泛空话（"做得不错""继续努力"）
- 跳过三视角对抗审查
...
| ---- | ---- |
| "代码没问题就不用查合规" | 合规是独立维度 |
| "小问题我自己修更快" | P0/P1 必须路由 |
| "规则很简单不用写示例" | 消费方契约必须三要素齐全 |
| "复盘没什么好写的" | 至少写一个具体改进点 |
...
- [ ] 资产更新满足消费方契约（每条规则有触发条件 + 可执行指令 + 示例）
- [ ] 复盘文档包含"本次沉淀的新规则"段落
- [ ] 8 类内容覆盖已检查（业务术语/架构/代码结构/接口/编码规范/测试/Review/交付）
- [ ] 工具适配产物 ≥ 2 类
